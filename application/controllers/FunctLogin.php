			...
			if ($password==$prosesLog->password) {
				if ($prosesLog->idLevel==1) {
					...
					$data = array(
						"id"=>$prosesLog->id,
						"username"=>$prosesLog->userId,
						"nama"=>$prosesLog->namaLengkap,
						"tempatLahir"=>$prosesLog->tempatLahir,
						"tglLahir"=>$prosesLog->tglLahir,
						"asalKota"=>$prosesLog->asalKota,
						"angkatan"=>$prosesLog->angkatan,
						"namaOrtu" => $prosesLog->namaOrtu,
						"alamatOrtu" => $prosesLog->alamatOrtu ,
						"kotaOrtu" => $prosesLog->kotaOrtu ,
						"provinsiOrtu" => $prosesLog->propinsiOrtu ,
						"alamat"=>$prosesLog->alamatLengkap,
						"email"=>$prosesLog->emailAktif,
						"noTelp"=>$prosesLog->noTelp,
						"status"=>$prosesLog->status);
					$this->session->set_userdata($data);
